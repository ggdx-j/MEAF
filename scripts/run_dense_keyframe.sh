...
CONFIG_FILE="${CONFIG_FILE:-${ROOT_DIR}/config/dense_keyframe.json}"
OUTPUT_FILE="${OUTPUT_FILE:-${ROOT_DIR}/outputs/dense_keyframe_3108_answer_only.jsonl}"
...
cd "${ROOT_DIR}"
"${PYTHON_BIN}" src/dense_keyframe_infer.py \
  --qa-file "${QA_FILE}" \
  --video-dir "${VIDEO_DIR}" \
