...
CONFIG_FILE="${CONFIG_FILE:-${ROOT_DIR}/config/duration_adaptive.json}"
OUTPUT_FILE="${OUTPUT_FILE:-${ROOT_DIR}/outputs/duration_adaptive_3108_answer_only.jsonl}"

export PYTHONPATH="${ROOT_DIR}:${PYTHONPATH:-}"
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

# httpx accepts SOCKS proxies as socks5://, while this host exports socks://.
...
cd "${ROOT_DIR}"
"${PYTHON_BIN}" src/duration_adaptive_infer.py \
  --qa-file "${QA_FILE}" \
  --video-dir "${VIDEO_DIR}" \
